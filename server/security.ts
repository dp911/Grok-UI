...
export class SecurityGate {
  ...
  headers: express.RequestHandler = (_request, response, next) => {
    ...
    response.setHeader(
      ...
      "default-src 'self'; base-uri 'none'; frame-ancestors 'none'; frame-src 'self' http://127.0.0.1:* http://localhost:*; form-action 'self'; img-src 'self' data:; font-src 'self'; style-src 'self'; script-src 'self'; connect-src 'self'",
    )
